...
def get_links_ODE(VIM, gene_names, regulators, sort=True, file_name=None):
...
def get_importances_ODE(TS_data, time_points, alpha="from_data", SS_data=None, gene_names=None, regulators='all',
                        param={}):
    ...
    for i in range(ngenes):
        ...
        if i in input_idx:
            input_idx.remove(i)
        vi = get_importances_single_ODE(TS_data, time_points, alphas[i], input_idx, i, SS_data, param)
        VIM[i, :] = vi
...
def get_importances_single_ODE(TS_data, time_points, alpha, input_idx, output_idx, SS_data, param):
...
# 计算最终的 score
def get_final_score_ODE(gene_fake, gene_real, SS_data_1, SS_data_2):
 
    # get the steady-state data
    SS_data_1 = pd.read_csv(SS_data_1, sep='\t').values
    SS_data_2 = pd.read_csv(SS_data_2, sep='\t').values
    SS_data = np.vstack([SS_data_1, SS_data_2])

    # get time series data
    TS_data = pd.read_csv(gene_fake, sep='\t').values
    gold_edges = pd.read_csv(gene_real, '\t', header=None)

    i = np.arange(0, 85, 21)
    j = np.arange(21, 106, 21)
    TS_data = [TS_data[i:j] for (i, j) in zip(i, j)]  # get the time-series data
    time_points = [np.arange(0, 1001, 50)] * 5  # get time points
    ngenes = TS_data[0].shape[1]
    gene_names = ['G' + str(i + 1) for i in range(ngenes)]

    regulators = gene_names.copy()
    xgb_kwargs = dict(n_estimators=100,
                      learning_rate=0.1654,
                      importance_type="weight",
                      max_depth=5,
                      n_jobs=-1)
    VIM = get_importances_ODE(TS_data,
                              time_points,
                              alpha=0.0122,
                              SS_data=SS_data,
    ...
    score_ODE = get_links_ODE(VIM, gene_names, regulators, sort=True, file_name=None)
